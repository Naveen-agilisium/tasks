...
function VideoPlayer() {
  ...
  const [playId, setPlayId] = useState(movieList.categories[0].videos[0])

  const handleVideo = (vidId) => {
     movieList.categories[0].videos.map((value) => {
      if(value.isPlay == true){
        value.isPlay = false;
      }
      return value;
    });
    let videoPlayer = videoInput.current;
    videoPlayer.pause();
    let clickTrigger = movieList.categories[0].videos[vidId];
    clickTrigger.isPlay = true;
    setPlayId(clickTrigger);
    videoPlayer.load();
    videoPlayer.play();
    ...
    <>
      <div className='videoPlayContainer'>
        <div className='vidPlayer'>
          <video autoPlay id="videoPlayer" ref={videoInput} controls>
            <source src={playId.sources[0]} type="video/mp4" />
          </video>
          <p id="vidTitle">{playId.title}</p>
          <p id="vidDesc">{playId.description} </p>
        </div>


        <div className='vidList'>
          {movieList.categories[0].videos.map((value, index) => (
            <div className={value.isPlay ? "vidCard played" :"vidCard"} key={index} onClick={() => handleVideo(index)}>
              <div className='vidImg'>
                <img src={value.thumb} />
                <div className="playIcon">
                <AiFillPlayCircle></AiFillPlayCircle></div>
              </div>
